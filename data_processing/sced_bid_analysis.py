...
- Streaming aggregations (sums/counts) instead of building giant long tables
- Ignores missing timestamps per file (intersects ts columns at read time)
- Reservoir sampling for violin plots

Expected INPUT_DIR contents:
  aggregation_HSL.csv
...
TS_BATCH_SIZE    = 200  # how many timestamp columns to process at once (lower if RAM tight)
TIMESTAMP_TZ     = None # e.g., "America/Chicago" or None for naive

# Violin sampling cap (per Resource Type × Step)
MAX_VIOLIN_SAMPLES_PER_GROUP = 8000

# Toggle which outputs to make
...
def list_timestamp_columns(sample_path: Path) -> List[str]:
    """Read only header to get timestamp columns (besides id vars)."""
    df = pd.read_csv(sample_path, nrows=1)
    if "Resource Node" in df.columns:
        df = df.rename(columns={"Resource Node": "Resource Name"})
...
# ---------- Column-batched reader (with intersection) ---------- #
def read_metric_batch(path: Path, ts_cols: List[str], value_name: str) -> pd.DataFrame:
    """
    Read id vars + the *present* subset of ts_cols for one metric file.
    If none of the requested ts_cols are present, return an empty frame
    with the expected schema so callers can safely skip.
    """
    # Check available columns from header
    hdr = pd.read_csv(path, nrows=0)
    cols_in_file = hdr.columns.tolist()

    rename_map = {}
    if "Resource Node" in cols_in_file:
        rename_map["Resource Node"] = "Resource Name"
        cols_in_file = [rename_map.get(c, c) for c in cols_in_file]

    id_vars = ["Resource Name", "Resource Type"]
    if not all(iv in cols_in_file for iv in id_vars):
        raise ValueError(f"{path.name}: missing required id columns {id_vars}")

    present_ts = [c for c in ts_cols if c in cols_in_file]
    if len(present_ts) == 0:
        # Return empty with expected columns; caller will skip
        return pd.DataFrame(columns=["Resource Name","Resource Type","timestamp", value_name])

    usecols_set = set(id_vars + present_ts)
    df = pd.read_csv(path, usecols=lambda c: (c in usecols_set))
    if rename_map:
        df = df.rename(columns=rename_map)

    long_df = df.melt(id_vars=id_vars, value_vars=present_ts,
...
class Reservoir:
    """Fixed-size reservoir per group using Vitter's Algorithm R."""
...
def ingest_and_stream(input_dir: Path, max_steps: int = 35):
    ...
    for ts_cols in batched(ts_cols_all, TS_BATCH_SIZE):
        ...
        for s in steps:
            ...
            # Skip if this step has no overlapping timestamp columns in this batch
            if mw_long.empty or pr_long.empty:
                continue
            ...
            for (rt, mo, hr), grp in m.groupby(["Resource Type","month","hour"]):
                key_bar = (int(mo), int(hr), int(s), str(rt))
                acc_add(mwfrac_sum_mhst, mwfrac_cnt_mhst, key_bar, grp["MW_frac"].to_numpy(dtype=float))

            # 2) Heatmaps: by (type, step, month, hour)
            for (rt, mo, hr), grp in m.groupby(["Resource Type","month","hour"]):
                key_hm = (str(rt), int(s), int(mo), int(hr))
                acc_add(price_sum_tsmh, price_cnt_tsmh, key_hm, grp["Price"].to_numpy(dtype=float))

            # 3) Single lines: by (type, step)
...
def main():
    ...
    # Save aggregates (useful for reuse/debugging)
