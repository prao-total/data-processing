...
def aggregate_base_point_matrix(
    ...
    for src_name, df in dfs.items():
        ...
        # Attempt to locate an optional resource type column
        rtype_env = env_get("RESOURCE_TYPE_COL", "Resource Type")
        rtype_col = None
        if rtype_env in df.columns:
            rtype_col = rtype_env
        else:
            stripped = {str(c).strip(): c for c in df.columns}
            if rtype_env in stripped:
                rtype_col = stripped[rtype_env]
            else:
                lower_map = {str(c).strip().lower(): c for c in df.columns}
                key = str(rtype_env).strip().lower()
                if key in lower_map:
                    rtype_col = lower_map[key]
        ...
        for row in work.itertuples(index=False, name=None):
            ...
            rt = row[3] if ('rtype_col' in locals() and rtype_col and len(row) > 3) else None
            if rt is not None and str(rt).strip() != "":
                if res not in res_type:
                    res_type[res] = rt
                elif res_type[res] in (None, "", float('nan')) and rt:
                    res_type[res] = rt
                elif res_type[res] != rt:
                    # Keep the first seen; you can switch to overwrite by ON_CONFLICT if desired
                    pass
    ...
    # Attach Resource Type (first column after index) if available
    try:
        if res_type:
            rt_series = pd.Series(res_type, name="Resource Type")
            rt_aligned = rt_series.reindex(agg_df.index)
            agg_df.insert(0, "Resource Type", rt_aligned)
    except Exception:
        pass
